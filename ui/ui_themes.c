// This file was generated by SquareLine Studio
// SquareLine Studio version: SquareLine Studio 1.4.2
// LVGL version: 9.1.0
// Project name: SquareLine_Project

#include "ui.h"


const ui_theme_variable_t _ui_theme_color_color1[1] = {0x218CFE};
const ui_theme_variable_t _ui_theme_alpha_color1[1] = {255};

const ui_theme_variable_t _ui_theme_color_color2[1] = {0x208BFE};
const ui_theme_variable_t _ui_theme_alpha_color2[1] = {180};
uint8_t ui_theme_idx = UI_THEME_DEFAULT;


void ui_theme_set(uint8_t theme_idx)
{
    ui_theme_idx = theme_idx;
    _ui_theme_set_variable_styles(UI_VARIABLE_STYLES_MODE_FOLLOW);
}

